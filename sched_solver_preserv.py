...
def wrap(x):
    ...
    for x in y:
        ...
        rets.append((np.cumsum(rews) + score))
    return (i, j), rets
...
def wrap_np(x):
    ...
    for x in y:
        _sample, num_proc, order, use_deadline = x
        score, rews = cy_heu.test_Lee(_sample, num_proc, order, use_deadline=use_deadline, ret_score=2)
        rets.append((np.cumsum(rews) + score))
    return (i, j), rets
...
class Solver(nn.Module):
    def __init__(self, num_proc, embedding_size, hidden_size, seq_len, tanh_exploration=10, use_deadline=False, nimp=False, use_cuda=False):
    ...
    def foward_np(self, inputs, argmax=False, get_reward=True, guide=None):
        """
        Args:
            inputs: [batch_size, seq_len, 3] (T, C, D)
        """
